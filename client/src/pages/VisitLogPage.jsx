...
import { MdAdd, MdSchool, MdPerson, MdPhone, MdDelete, MdCameraAlt, MdClose, MdCheckCircle, MdLocationOn, MdNote } from 'react-icons/md';
 ...
 const handleSubmit = () => {
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '20px 16px', fontFamily: 'Inter,sans-serif' }}>
              ...
              <motion.div key={visit._id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                ...
                <div onClick={() => setExpandedId(expanded ? null : visit._id)}
                  ...
                  <div style={{ width: 46, height: 46, borderRadius: 12, overflow: 'hidden', flexShrink: 0, background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {visit.selfieUrl ? <img src={visit.selfieUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 22 }}>🏫</span>}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2, flexWrap: 'wrap' }}>
                      <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: '#111827', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 180 }}>{visit.schoolName}</p>
                  ...
                  {expanded && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} style={{ overflow: 'hidden' }}>
                      <div style={{ padding: '0 16px 16px', borderTop: '1px solid #F9FAFB' }}>
                        {visit.selfieUrl && <img src={visit.selfieUrl} alt="" style={{ width: '100%', maxHeight: 200, objectFit: 'cover', borderRadius: 12, margin: '12px 0' }} />}
      ...
      <AnimatePresence>
        {modalOpen && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', padding: '0' }}>
            {/* Backdrop */}
              ...
              <div style={{ padding: '0 20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div style={{ background: '#F9FAFB', borderRadius: 14, padding: 16, border: '1.5px dashed #E5E7EB' }}>
                  <p style={{ margin: '0 0 10px', fontSize: 12, fontWeight: 600, color: '#6B7280', textTransform: 'uppercase', letterSpacing: 0.5 }}>
                    📸 Selfie with Teacher/Principal <span style={{ color: '#9CA3AF', fontWeight: 400, textTransform: 'none' }}>(optional)</span>
                  </p>
                    ...
                    {OUTCOMES.map(o => (
                      <button key={o.value} type="button" onClick={() => setForm(f => ({ ...f, outcome: o.value }))}
                        style={{
                          ...
                          background: form.outcome === o.value ? o.bg : '#fff',
                          color: form.outcome === o.value ? o.color : '#9CA3AF',
                          border: `1.5px solid ${form.outcome === o.value ? o.border : '#E5E7EB'}`
                        }}>
                        {o.emoji} {o.label}
                      </button>
                    ))}
